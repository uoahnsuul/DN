...
class checkpoint():
    ...
    def initCSVFile(self):
        filename = os.path.join('./result.csv')
    ...
    def writeCSVFile(self, best, latest, anc, epoch):
        data = {'date': str(datetime.datetime.now().strftime('%Y-%m-%d-%H-%M-%S')),
    ...
    def bg_target(queue):
        while True:
            if not queue.empty():
                ...
                img = Image.frombytes('YCbCr', (tensor.shape[1], tensor.shape[0]),
                                      np.ascontiguousarray(tensor.numpy())).convert('RGB')
