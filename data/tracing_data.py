...
class tracing_data(srdata.SRData):
    ...
    def read_npz_file(self, file):
        ...
        if self.args.image_pin_memory:
            if file not in self.image_dic:
                self.image_dic[file] = np.load(file,allow_pickle=True)
    ...
    def np2tensor(self, args, rgb_range=1023):
        def _np2Tensor(img):
            ...
            tensor[0:3,:,:]=tensor[0:3,:,:].mul_(1 / rgb_range)
            return tensor
    ...
    def getBlock2dCu(self, y, dy, x, dx, depth, idx, name, value_idx = 0):
        if self.args.sc:
            block2d = np.full((dy//2, dx//2), np.nan)
        else:
            block2d = np.zeros((dy, dx))
        ...
        for _xp, _yp, _w, _h, *values in filtered.T:
            ...
            while j < _yp+_h:
                ...
                j += 1
            sum /= (_w*_h)
            block2d[_yp:_yp+_h, _xp:_xp+_w] = sum

        return block2d

    def __getitem__(self, idx):
        lr, hr, filename, idx = self._load_file(idx)
